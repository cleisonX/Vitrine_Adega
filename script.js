...
if (isFirebaseConfigured) {
    ...
    dataLayer = {
        ...
        subscribe(cb) {
            ...
            onSnapshot(q, (snap) => { cb(snap.docs.map(d => ({ id: d.id, ...d.data() }))); });
        },
    ...
    function readLS() { try { return JSON.parse(localStorage.getItem(LS_PRODUCTS)) || []; } catch { return []; } }
    ...
    dataLayer = {
        ...
        async updateProduct(id, updates, imageFile) {
            ...
            if (index !== -1) { list[index] = { ...list[index], ...updates, imageUrl }; writeLS(list); this.notify(); }
        },
        async deleteProduct(id) { writeLS(readLS().filter(p => p.id !== id)); this.notify(); },
...
if (!isFirebaseConfigured) {
    ...
    if (bannerWrap) bannerWrap.innerHTML = `<div class="demo-banner"><span>⚠️</span><div><b>Modo demonstração.</b> Adicione <code>?adm</code> na URL para gerenciar.</div></div>`;
}
...
function renderCategories() {
        ...
        if (isActive && cat !== "Todos") {
            ...
            if (brands.size > 0) {
                brandsHtml = `<ul class="brand-sublist">
                    ...
                    ${Array.from(brands).sort().map(brand => `<li><button class="cat-btn sub-btn ${brand === activeBrand ? "active" : ""}" data-brand="${escapeHtml(brand)}"><span>${escapeHtml(brand)}</span></button></li>`).join("")}
                </ul>`;
            }
        }
        return `<li>
    ...
    document.querySelectorAll("[data-cat]").forEach(btn => btn.addEventListener("click", () => {
        ...
        if (window.innerWidth <= 1023) { if (cat === "Todos" || !hasBrands) closeSidebar(); }
        renderAll();
    }));
...
function loadCart() { try { cart = JSON.parse(localStorage.getItem(CART_STORAGE_KEY)) || []; } catch { cart = []; } updateCartUI(); }
function saveCart() { localStorage.setItem(CART_STORAGE_KEY, JSON.stringify(cart)); updateCartUI(); }
...
function renderCartModal() {
    ...
    container.innerHTML = cart.map(item => {
        ...
        return `<div class="cart-item">
            ...
            <div class="cart-item-info"><div class="cart-item-name">${escapeHtml(item.name)}</div><div class="cart-item-price">${money(item.price)}</div></div>
    ...
    toggleDeliveryFields(); togglePaymentFields();
}
...
function checkoutCart() {
    ...
    let changeMessage = '';
    if (paymentMethod?.value === 'dinheiro' && changeAmount?.value) changeMessage = `Troco para: ${money(parseFloat(changeAmount.value))}`;
    let total = 0;
    ...
    setTimeout(() => { cart = []; saveCart(); renderCartModal(); closeModal('cartModal'); toast('✅ Pedido enviado!', 'ok'); }, 1000);
}
...
window.addToCart = function (id, name, price, image, fromModal = false) {
    ...
    else toast(`✅ ${name} adicionado ao carrinho!`, 'ok');
};
window.removeFromCart = function (id) {
    const existing = cart.find(i => i.id === id);
    if (existing) { if (existing.qty > 1) existing.qty -= 1; else cart = cart.filter(i => i.id !== id); }
    saveCart(); renderCartModal();
};
window.removeItemCompletely = function (id) { cart = cart.filter(i => i.id !== id); saveCart(); renderCartModal(); };
...
//EVENTOS CARRINHO
document.getElementById('cartFloat')?.addEventListener('click', function (e) { e.preventDefault(); renderCartModal(); openModal('cartModal'); });
...
function renderGrid() {
    ...
    grid.innerHTML = list.map(p => {
        ...
        const availableBadge = p.available === 'indisponivel' ? `<span class="unavailable-badge">❌ Indisponível</span>` : '';
        const isAvailable = p.available !== 'indisponivel';
        const cartButton = isAvailable
            ? `<button class="btn-add-cart" onclick="window.addToCart('${p.id}', '${safeName}', ${p.price}, '${imageUrl}')">Adicionar ao carrinho </button>`
            : `<button class="btn-add-cart btn-unavailable" disabled>❌ Indisponível</button>`;
        const descHtml = safeDesc ? `<p class="card-desc rotate-text">${safeDesc}</p>` : `<p class="card-desc" style="visibility:hidden;">&nbsp;</p>`;
...
function openEditModal(productId) {
    ...
    document.getElementById('productAvailable').value = product.available || 'disponivel';
...
function renderAdminList() {
    ...
    box.innerHTML = PRODUCTS.map(p => {
        ...
        const availBadge = p.available === 'indisponivel' ? ' ❌' : '';
        return `<div class="admin-row">
            <img src="${p.imageUrl || placeholderImg()}" alt="${escapeHtml(p.name)}">
            <div class="info">
                <div class="n">${escapeHtml(p.name)}${promoBadge}${availBadge}</div>
    ...
    document.querySelectorAll("[data-edit2]").forEach(btn => btn.addEventListener("click", () => openEditModal(btn.dataset.edit2)));
}

//CATEGORIA DE SELECÃO
...
document.getElementById("productForm")?.addEventListener("submit", async (e) => {
    ...
    const available = document.getElementById("productAvailable")?.value || "disponivel";
    ...
    try {
        const productData = { name, price, category, description, brand, promo, available };
        if (editingProductId) {
